...
def main():
    ...
    if len(image_points) > 0:
        # カメラ内部パラメータを計算
        print('omnidir.calibrate()')

        calibration_flags = cv.omnidir.CALIB_USE_GUESS + cv.omnidir.CALIB_FIX_SKEW + cv.omnidir.CALIB_FIX_CENTER
        ...
        rms, K, xi, d, rvecs, tvecs, idx = \
            cv.omnidir.calibrate(
                ...
                criteria=(cv.TERM_CRITERIA_EPS + cv.TERM_CRITERIA_MAX_ITER, 200, 1e-8)
            )
        print("RMS = " + str(rms))
        ...
        np.savetxt(xi_filename, xi, delimiter=',', fmt="%0.14f")  # xiの保存
    else:
