...
| | なぜ |
| --- | --- |
| **`--images` を足した** | **走行体は SSH 越しで画面が無い。** 本家は `cv.imshow` / `waitKey` が要る。撮るのは別にして、画像の束から解けるようにした |
...
```bash
# 1. 走行体で撮る（画面が要らない。盤が写った枚だけ残る）
#    sorot_spike/Tools/capture_chessboard.py

# 2. 母艦で解く
python 01-01_calibrateCamera.py --images 'chess/*.png' \
    --square_len 24.0 --grid_size 9,6
```

**`--grid_size` は「内側の角」の数。マスの数ではない**（10x7 マスの盤は `9,6`）。
