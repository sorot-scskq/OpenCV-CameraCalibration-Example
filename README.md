...
| | なぜ |
| --- | --- |
| **動画1本を既定の使い方にした** | **走行体は SSH 越しで画面が無い。** 本家は `cv.imshow` / `waitKey` が要り、一度も動かせない。動画・画像・フォルダを位置引数で受け、姿勢の違う枚だけ選って採る |
